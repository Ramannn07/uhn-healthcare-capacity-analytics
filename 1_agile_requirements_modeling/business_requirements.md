...
Uncoordinated patient management has increased check-in-to-assessment wait times by 35%, and annual nursing overtime expenditures have breached baseline operational budgets by 22%. This project introduces a modular, automated information system to ingest raw triage logs, calculate real-time asset utilization metrics, and reduce executive decision-making latency by 30%.
...
* **User Story 1:**
  ...
  * **Acceptance Criteria:**
    * The analytical layer must dynamically compute wait time as the difference between arrival_timestamp and physician_initial_assessment_timestamp.
...
* Rule ID: BR-001
  * Data Element: Arrival_Time
  * Ingestion Rule: Must contain a valid ISO timestamp. Cannot be NULL or blank.
  * Failure Action: Reject transaction entirely; route row to System Error Log.

* Rule ID: BR-002
  * Data Element: Triage_Acuity
  * Ingestion Rule: Must be an integer between 1 (Resuscitation) and 5 (Non-Urgent) following the Canadian Triage and Acuity Scale (CTAS).
  * Failure Action: Flag record as 'Incomplete Triage' and isolate for audit.

* Rule ID: BR-003
  * Data Element: Discharge_Time
  * Ingestion Rule: If missing or NULL, the patient is classified as currently active in a physical bed.
  * Failure Action: Dynamic Imputation: Programmatically apply the active execution running snapshot timestamp.

* Rule ID: BR-004
  * Data Element: Bed_Utilization_Rate_Pct
  * Ingestion Rule: Any value calculated strictly above 85.00% is classified as a critical system strain.
  * Failure Action: Trigger a conditional UI alert flag (CRITICAL_OVERFLOW_RISK).

---

## 5. System Data Lineage
The system's data pathway functions via four modular abstraction layers:
1. Ingestion Layer: Raw comma-separated transaction payloads (uhn_raw_triage_logs.csv) are fetched by the data engine.
